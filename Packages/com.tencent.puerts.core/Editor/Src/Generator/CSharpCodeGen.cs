...
";
    ...
    {SToCPPType(signature)} {csName} = {GetArgValue(S, jsName, true)};";
            ...
            else if (signature[0] == 'V')
            {
                ...
                int start = int.Parse(Regex.Match(jsName, @"_sv(\d+)").Groups[1].Value);
                if (IsPrimitive(si))
                {
...
";
        ...
        public static string GenExtensionMethodInfos(List<KeyValuePair<Type, List<Type>>> extendedType2extensionType)
        {
            ...
            {
                string extendedTypeName = TypeExtensions.GetFriendlyName(pair.Key);
                var extensionTypeNames = pair.Value.Select(t => $"typeof({TypeExtensions.GetFriendlyName(t)})").ToList();
