...
public class EditingLevelManager : MonoBehaviour
{
	...
	{
		EventManager.Trigger(IsPaused ? EventManager.EventName.Resume : EventManager.EventName.Pause);
	}
	...
	public void SaveProject()
	{
		...
		File.WriteAllText(Path.Combine(LevelPath, $"{SingleSetting.Difficulty}.dlf"),
			RawChartInfo.ToChartInfo().ToSimpleDLF(layerInfo));
	...
	{
		File.WriteAllText(
			Path.Combine(LevelPath, "Takana_Data", "AutoSave",
				$"{SingleSetting.Difficulty}_{DateTime.Now:yyyy-MM-dd-HH-mm-ss}.dlf"),
			RawChartInfo.ToChartInfo().ToSimpleDLF());
		Debug.Log("自动保存谱面");
	}
