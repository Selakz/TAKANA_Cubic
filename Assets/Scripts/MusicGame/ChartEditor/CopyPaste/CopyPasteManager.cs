...
public class CopyPasteManager : MonoBehaviour
{
    ...
    public void CopyTurningPoints(List<(float time, float x, string curve)> leftPoints, List<(float time, float x, string curve)> rightPoints, Track fromTrack)
    {
        Clear();
        templateLeftPoints = new(leftPoints);
        templateRightPoints = new(rightPoints);
        originalTrack = fromTrack;
        HeaderMessage.Show($"复制成功！已复制{templateLeftPoints.Count}个左结点，{templateRightPoints.Count}个右结点", HeaderMessage.MessageType.Success);
    }
        ...
        try
        {
            foreach (BaseNote note in templateBaseNotes)
            {
                var command = new CreateNoteCommand(
                    note.Clone(EditingLevelManager.Instance.RawChartInfo.NewId, time + note.TimeJudge - baseTime, track));
    ...
    public void PasteTracks(float time, float left)
    {
        ...
        try
        {
            ...
            {
                Track newTrack = track.Clone(EditingLevelManager.Instance.RawChartInfo.NewId, time + track.TimeInstantiate - baseTime, left + track.GetX(track.TimeInstantiate, true) - baseLeft);
    ...
    public void PasteTurningPoints(float time)
    {
        ...
        try
        {
            ...
            {
                var command = new MoveListInsertCommand(track.LMoveList, (time + item.time - baseTime, trackX + item.x - originalX, item.curve));
                commands.Add(command);
            }
            foreach (var item in templateRightPoints)
            {
                var command = new MoveListInsertCommand(track.RMoveList, (time + item.time - baseTime, trackX + item.x - originalX, item.curve));
                commands.Add(command);
            }
        }
        catch
    ...
    public void PasteExactBaseNotes(float time)
    {
        ...
        try
        {
            ...
            {
                var command = new CreateNoteCommand(
                    note.Clone(EditingLevelManager.Instance.RawChartInfo.NewId, time + note.TimeJudge - baseTime, note.BelongingTrack));
                commands.Add(command);
            }
        }
        catch
    ...
    public void PasteExactTracks(float time)
    {
        ...
        try
        {
            foreach (var track in templateTracks)
            {
                Track newTrack = track.Clone(EditingLevelManager.Instance.RawChartInfo.NewId, time + track.TimeInstantiate - baseTime, track.GetX(track.TimeInstantiate, true));
                var command = new CreateTrackCommand(newTrack);
                commands.Add(command);
                if (templateTracks.Count == 1) if1ThenTheId = newTrack.Id;
            }
    ...
    public void PasteExactTurningPoints(float time)
    {
        ...
        try
        {
            ...
            foreach (var item in templateLeftPoints)
            {
                var command = new MoveListInsertCommand(track.LMoveList, (time + item.time - baseTime, item.x, item.curve));
                commands.Add(command);
            }
            foreach (var item in templateRightPoints)
            {
                var command = new MoveListInsertCommand(track.RMoveList, (time + item.time - baseTime, item.x, item.curve));
                commands.Add(command);
            }
        }
        catch
