...
namespace MusicGame.ChartEditor.TrackLine
{
	public static class LineDrawer
	{
		...
		public static void DrawStraightLine(Mesh mesh, float precision)
		{
			List<Vector2> points = new();
			for (float i = 0; i < 1 - 1e-4f; i += precision)
			{
				points.Add(new(0, i));
			}

			points.Add(new(0, 1 - 1e-4f));
			points.Add(new(0, 1));
			PointsToMesh(mesh, points.ToArray(), 0.02f);
		}
