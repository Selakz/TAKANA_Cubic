...
public class TurningPoint : MonoBehaviour
{
	...
	public void Initialize(BaseTrackMoveList moveList, int index)
	{
		...
		if (!IsEnd)
		{
			...
			LineDrawer.DrawCurve(lineRenderer, new(Camera.main.G2WPosX(startX), Camera.main.G2WPosY(startY)),
				new(Camera.main.G2WPosX(endX), Camera.main.G2WPosY(endY)), moveList[index].curve, 0.15f);
			LineDrawer.DrawCurve(edgeCollider, new(Camera.main.G2WPosX(startX), Camera.main.G2WPosY(startY)),
				new(Camera.main.G2WPosX(endX), Camera.main.G2WPosY(endY)), moveList[index].curve, 0.1f);
		}
	}
	...
	private string GetNextCurveLabel(string label)
	{
		...
		{
			"u" => "s",
			"s" => CurveCalculator.GetEaseById(TrackLineManager.Instance.CurrentEaseId * 10 + 2).GetString(),
			_ => label[1] switch
			{
				'i' => label[0] + "o",
				'o' => label[0] + "b",
	...
	void Update()
	{
		...
		if (IsSelected)
		{
			...
			if (InputManager.Instance.IsHotkeyActionPressed(InputManager.Instance.InScreenEdit.ToLeftGrid))
			{
				...
				{
					var original = moveList[index];
					var updated = (original.time, GridManager.Instance.GetNearestXGridPos(original.x - 0.001f).left,
						original.curve);
					CommandManager.Instance.Add(new MoveListUpdateCommand(moveList, original, updated));
				}
			}
			...
			if (InputManager.Instance.IsHotkeyActionPressed(InputManager.Instance.InScreenEdit.ToRightGrid))
			{
				...
				{
					var original = moveList[index];
					var updated = (original.time, GridManager.Instance.GetNearestXGridPos(original.x + 0.001f).right,
						original.curve);
					CommandManager.Instance.Add(new MoveListUpdateCommand(moveList, original, updated));
				}
			}
			...
			{
				var original = moveList[index];
				var updated = (original.time, -original.x, original.curve);
				CommandManager.Instance.Add(new MoveListUpdateCommand(moveList, original, updated));
			}
