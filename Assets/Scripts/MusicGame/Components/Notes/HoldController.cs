	...
	public void HandleFail()
	{
		if (!isFailed)
		{
			    ...
			    || (!isJudged && Current > _hold.TimeJudge + _hold.JudgeInfo.TimeMiss))
			{
				...
				EventManager.Trigger(EventManager.EventName.AddJudgeAndInputInfo,
					(JudgeResult.LateMiss, (InputInfo)realTimeInfo));
	...
	void Update()
	{
		if (!isPaused)
		{
			...
			else if (Current > _hold.TimeEnd)
			{
				...
				{
					EventManager.Trigger(EventManager.EventName.AddJudgeAndInputInfo,
						(JudgeResult.CriticalPerfect, (InputInfo)realTimeInfo));
					var effect = Instantiate(hitEffect);
					effect.transform.position = effect.transform.position = new(transform.position.x,
						_hold.BelongingTrack.BelongingLine.ThisObject.transform.position.y);
					isJudged = false;
				}
