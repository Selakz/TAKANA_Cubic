...
namespace DynamicPanels
{
	...
	public class DynamicPanelsCanvas : MonoBehaviour, IPointerEnterHandler, ISerializationCallbackReceiver
	{
		internal class InternalSettings
		{
			...
			public Panel DummyPanel
			{
				get { return canvas.dummyPanel; }
			}
...
#pragma warning disable 0649
		[SerializeField] private bool m_leaveFreeSpace = true;
		...
		[SerializeField] private Vector2 minimumFreeSpace = new Vector2(50f, 50f);

		[SerializeField] private RectTransform freeSpaceTargetTransform;
		private Vector2 freeSpacePrevPos, freeSpacePrevSize;

		public bool PreventDetachingLastDockedPanel;

		[SerializeField] private float m_panelResizableAreaLength = 12f;

		public float PanelResizableAreaLength
		{
			get { return m_panelResizableAreaLength; }
		}

		[SerializeField] private float m_canvasAnchorZoneLength = 20f;

		public float CanvasAnchorZoneLength
		{
			get { return m_canvasAnchorZoneLength; }
		}

		[SerializeField] private float m_panelAnchorZoneLength = 100f;

		public float PanelAnchorZoneLength
		{
			get { return m_panelAnchorZoneLength; }
		}

		private const float m_panelAnchorZoneLengthRatio = 0.31f;

		public float PanelAnchorZoneLengthRatio
		{
			get { return m_panelAnchorZoneLengthRatio; }
		}

		[SerializeField] private List<PanelProperties> initialPanelsUnanchored;
...
#pragma warning restore 0649
		...
		private void Start()
		{
			...
			HashSet<Transform>
				createdTabs =
					new HashSet<Transform>(); // A set to prevent duplicate tabs or to prevent making canvas itself a panel
		...
		private void CreateAnchoredPanelsRecursively(List<AnchoredPanelProperties> anchoredPanels, Panel rootPanel,
			HashSet<Transform> createdTabs, Dictionary<Panel, Vector2> initialSizes)
		{
			...
			{
				Panel panel = CreateInitialPanel(anchoredPanels[i].panel, rootPanel, anchoredPanels[i].anchorDirection,
					createdTabs);
				if (panel == null)
					panel = rootPanel;
		...
		private Panel CreateInitialPanel(PanelProperties properties, Panel anchor, Direction anchorDirection,
			HashSet<Transform> createdTabs)
		{
			Panel panel = null;
		...
		private void InitializeAnchorZones()
		{
			...
			CreateAnchorZone(Direction.Left, new Vector2(0f, 0f), new Vector2(0f, 1f),
				new Vector2(m_canvasAnchorZoneLength, 0f));
			CreateAnchorZone(Direction.Top, new Vector2(0f, 1f), new Vector2(1f, 1f),
				new Vector2(0f, m_canvasAnchorZoneLength));
			CreateAnchorZone(Direction.Right, new Vector2(1f, 0f), new Vector2(1f, 1f),
				new Vector2(m_canvasAnchorZoneLength, 0f));
			CreateAnchorZone(Direction.Bottom, new Vector2(0f, 0f), new Vector2(1f, 0f),
				new Vector2(0f, m_canvasAnchorZoneLength));
		...
		{
			CanvasAnchorZone anchorZone = new GameObject("AnchorZone" + direction, typeof(RectTransform))
				.AddComponent<CanvasAnchorZone>();
