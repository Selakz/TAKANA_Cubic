...
namespace DynamicPanels
{
	...
	public class DynamicPanelsCanvasEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			bool guiEnabled = !EditorApplication.isPlaying ||
			                  AssetDatabase.Contains(((DynamicPanelsCanvas)serializedObject.targetObject).gameObject);
			...
			else
			{
				...
				if (selectedAnchoredPanel != null)
				{
					...
					{
						string initialSizeLabel = selectedAnchoredPanel.initialSize == Vector2.zero
							? "Initial Size (not set):"
							: "Initial Size:";

						EditorGUI.BeginChangeCheck();
						Vector2 panelInitialSize =
							EditorGUILayout.Vector2Field(initialSizeLabel, selectedAnchoredPanel.initialSize);
					...
					if (selectedAnchoredPanelTabs != settings.InitialPanelsAnchored.panel.tabs)
					{
						...
						if (GUILayout.Button("Remove Selected",
							    GUILayout.Height(1.35f * EditorGUIUtility.singleLineHeight)))
				...
				if (rootDirection != Direction.None)
				{
					...
					settings.InitialPanelsAnchored.subPanels.Insert(0,
						new DynamicPanelsCanvas.AnchoredPanelProperties() { anchorDirection = rootDirection });
				}
			}
		...
		private bool RemoveAnchoredPanel(DynamicPanelsCanvas.AnchoredPanelProperties root,
			DynamicPanelsCanvas.AnchoredPanelProperties panel)
		{
		...
		{
			bool shouldDrawSelf = leaveFreeSpace.boolValue || props != settings.InitialPanelsAnchored ||
			                      props.subPanels == null || props.subPanels.Count == 0;
			...
			switch (ev.GetTypeForControl(controlID))
			{
				...
				case EventType.DragUpdated:
					...
					{
						DynamicPanelsCanvas.AnchoredPanelProperties drag =
							DragAndDrop.GetGenericData("props") as DynamicPanelsCanvas.AnchoredPanelProperties;
						if (drag == null)
						{
							int i;
								...
								for (int i = 0; i < draggedObjects.Length; i++)
								{
									...
									if (transform != null)
									{
										if (props.panel.tabs.Find((tab) => tab.content == transform) == null)
											props.panel.tabs.Add(new DynamicPanelsCanvas.PanelTabProperties()
												{ content = transform });
									}
								}
							}
		...
		private bool DrawReorderableListFor(DynamicPanelsCanvas.PanelProperties panelProperties)
		{
			...
			else
			{
				ReorderableList reorderableList =
					new ReorderableList(tabs, typeof(DynamicPanelsCanvas.PanelTabProperties), true, true, true, true)
					{
				...
				reorderableList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
				{
					...
					DrawReorderableListItem(rect, index,
						(DynamicPanelsCanvas.PanelTabProperties)reorderableList.list[index]);
				};
		...
		private void DrawReorderableListItem(Rect rect, int index, DynamicPanelsCanvas.PanelTabProperties tab)
		{
			...
			Rect tabIconRect = new Rect(rect.x + LABEL_WIDTH + contentWidth * 0.5f, rect.y + lineHeight,
				contentWidth * 0.5f, lineHeight);

			Rect minSizeLabelRect = new Rect(rect.x, rect.y + 2 * lineHeight, LABEL_WIDTH, lineHeight);
			Rect minSizeXRect = new Rect(rect.x + LABEL_WIDTH, rect.y + 2 * lineHeight, contentWidth * 0.5f,
				lineHeight);
			Rect minSizeYRect = new Rect(rect.x + LABEL_WIDTH + contentWidth * 0.5f, rect.y + 2 * lineHeight,
				contentWidth * 0.5f, lineHeight);
			...
			RectTransform content =
				EditorGUI.ObjectField(contentRect, GUIContent.none, tab.content, typeof(RectTransform), true) as
					RectTransform;
			if (EditorGUI.EndChangeCheck())
			{
			...
			EditorGUI.BeginChangeCheck();
			Sprite tabIcon =
				EditorGUI.ObjectField(tabIconRect, GUIContent.none, tab.tabIcon, typeof(Sprite), false) as Sprite;
			if (EditorGUI.EndChangeCheck())
			{
