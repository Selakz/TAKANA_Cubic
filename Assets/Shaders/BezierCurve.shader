Shader "Unlit/Bezier Plane Curve"
{
    Properties
    {
        ...
        _BezierFactor ("Bezier Factor", Vector) = (0,0,0,0)
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            float _Period;
            float _Amplitude;
            float4 _BezierFactor;
            float4 _MainTex_ST;

            inline float CubicBezier(float start, float startControl, float endControl, float end, float t)
            {
                float u = 1 - t;
                return u * u * u * start +
            ...
            inline float CubicDerivative(float start, float startControl, float endControl, float end, float t)
            {
                float u = 1 - t;
            ...
            inline float GetBezierValue(float x)
            {
                ...
                {
                    float currentT = CubicBezier(0, _BezierFactor.x, _BezierFactor.z, 1, factorT);
                    float slope = CubicDerivative(0, _BezierFactor.x, _BezierFactor.z, 1, factorT);
                    if (abs(slope) < 1e-5) break;

                    factorT -= (currentT - x) / slope;
                    factorT = clamp(factorT, 0, 1);
                }

                float positionT = CubicBezier(0, _BezierFactor.y, _BezierFactor.w, 1, factorT);
                return lerp(0, 1, positionT);
            }

            inline float2 GetBezierPoint(float x)
            {
                return float2(x, GetBezierValue(x)) * float2(_Amplitude, _Period);
            }

            float2 TransformedPoint(float x, float y, float precision)
            {
                float2 result;
                float2 centerPoint = GetBezierPoint(x);
                float width = _Width * (y > 0 ? 1 : -1);

                if (x < 1e-5f)
                {
                    float2 nextPoint = GetBezierPoint(x + precision);
                ...
                {
                    float2 lastPoint = GetBezierPoint(x - precision);
